...
class Sound {
  ...
  load(name = "standard") {
    ...
    this.noLoops = false
    if (
      name === this.lastLoaded &&
      settings.settings.nextSoundbank === this.lastNext &&
      settings.settings.voicebank === this.lastVoice
    ) {
      return
    }
    this.mustWait = true
    ...
    this.loadMenu()
    this.loadMenuVoice()
    loadSoundbank(name).then((soundData) => {
      this.doesSoundBankUseReadyGoVoices = soundData.usesReadyGoVoices
        ? true
        : false
        ...
        this.sounds[soundName] = new Howl({
          ...
          onend: () => {
            if (soundName == "topoutwarning") {
              ...
              // if (this.amountOfTimesEnded[soundName] > 50 && this.playingSeLoops[soundName] != null && this.fadedSounds[soundName] == null) {
              ...
              this.amountOfTimesEnded[soundName]++
              if (
                this.amountOfTimesEnded[soundName] > 2 &&
                this.playingSeLoops[soundName] != null &&
                this.fadedSounds[soundName] == null
              ) {
                this.fadedSounds[soundName] = true
                this.sounds[soundName].fade(
                  settings.settings.sfxVolume / 100,
                  (settings.settings.sfxVolume / 100) * 0.5,
                  500
                )
              }
            }
      ...
      if (
        (soundData.nextSoundbank != null ||
          settings.settings.nextSoundbank !== "auto") &&
        settings.settings.nextSoundbank !== "off"
      ) {
        const name =
          settings.settings.nextSoundbank !== "auto"
            ? settings.settings.nextSoundbank
            : soundData.nextSoundbank
        this.mustWait = true
        for (const piece of Object.keys(PIECES)) {
          ...
          this.sounds[soundName] = new Howl({
            ...
            onend: () => {
              this.amountOfTimesEnded[soundName]++
              if (
                this.amountOfTimesEnded[soundName] > 5 &&
                this.playingSeLoops[soundName] != null &&
                this.fadedSounds[soundName] == null
              ) {
                this.fadedSounds[soundName] = true
        ...
        loadPiecebank(name).then((nextData) => {
  ...
  playBgm(name, type) {
    ...
    if (gameHandler.game.settings.hasDangerBgm) {
      if (this.dangerBgmIsRaised) {
        this.music[`${type}-${name}-danger-start`].volume(
          settings.settings.musicVolume / 100
        )
        this.music[`${type}-${name}-danger-loop`].volume(
          settings.settings.musicVolume / 100
        )
      }
    ...
    if (gameHandler.game.settings.hasPaceBgm) {
      if (this.paceBgmIsRaised) {
        this.music[`${type}-${name}-pace-start`].volume(
          settings.settings.musicVolume / 100
        )
        this.music[`${type}-${name}-pace-loop`].volume(
          settings.settings.musicVolume / 100
        )
      }
  ...
  raiseDangerBgm() {
    if (
      !gameHandler.game.settings.hasDangerBgm ||
    ...
    if (!this.dangerBgmIsRaised) {
      this.syncBgm()
      this.music[`${this.dangerBgmName}-start`].fade(
        0,
        settings.settings.musicVolume / 100,
        500
      )
      this.music[`${this.dangerBgmName}-loop`].fade(
        0,
        settings.settings.musicVolume / 100,
        500
      )
  ...
  raisePaceBgm() {
    if (
      !gameHandler.game.settings.hasPaceBgm ||
    ...
    if (!this.paceBgmIsRaised) {
      this.syncBgm()
      this.music[`${this.paceBgmName}-start`].fade(
        0,
        settings.settings.musicVolume / 100,
        500
      )
      this.music[`${this.paceBgmName}-loop`].fade(
        0,
        settings.settings.musicVolume / 100,
        500
      )
  ...
  playSeQueue() {
    ...
    for (let name of Object.keys(this.toPlay)) {
      if (
        this.files.indexOf(name) === -1 &&
        name.substr(name.length - 4) === "mini"
      ) {
      ...
      if (this.files.indexOf(name) !== -1) {
        ...
        name === "initialskip" &&
        this.files.indexOf("initialhold") !== -1
      ) {
        this.sounds["initialhold"].play()
      } else if (
        name === "prespinmini" &&
        this.files.indexOf("prespin") !== -1
      ) {
        this.sounds["prespin"].play()
      } else if (name === "go" && this.files.indexOf("start") !== -1) {
        this.sounds["start"].play()
      } else if (
        (name === "skip" && this.files.indexOf("hold") !== -1) ||
        (name === "initialskip" &&
          this.files.indexOf("hold") !== -1 &&
          this.files.indexOf("initialhold") === -1)
      ) {
        this.sounds["hold"].play()
      }

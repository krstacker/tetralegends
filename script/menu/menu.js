...
class Menu {
  ...
  load(name, type = "default", menuData = null) {
    ...
    const render = (menu) => {
      ...
      this.current.lang = this.current.properties.langOverride
        ? this.current.properties.langOverride
        : `menu_${this.current.name}`
  ...
  draw() {
    ...
    for (let i = 0; i < this.current.data.length; i++) {
      ...
      if (currentData.secretA && !input.holdingCtrl && !this.skipSecretA) {
        // For those who are looking through my code, this is how you can access a secret game.
        continue
      ...
      if (currentData.type === "control") {
        ...
        if (currentData.langOverride) {
          element.textContent = locale.getString(
            currentData.langOverride,
            currentData.string,
            currentData.replace
          )
        } else {
